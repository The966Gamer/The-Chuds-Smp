...
export function McHead({
  ...
  const src = headUrl || `https://mc-heads.net/avatar/${encodeURIComponent(username || "Steve")}/${size * 2}`;
  if (failed) {
    return (
      <div
        className="mc-head"
        style={{
          ...
          fontSize: size * 0.5,
          color: "var(--text-dim)",
          background: "rgba(146, 176, 120, 0.15)",
...
export function EmptyState({
  ...
  return (
    <div className="empty-state">
      <div className="icon">
        <PixelIconView name={icon} size={34} style={{ opacity: 0.8 }} />
      </div>
