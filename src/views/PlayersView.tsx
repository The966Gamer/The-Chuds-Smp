      ...
      <div className="spread" style={{ marginBottom: 14 }}>
        <div>
          ...
          <p className="dim" style={{ margin: 0, fontSize: 13 }}>
            Everyone who has joined the server.
          </p>
        </div>
        <input
          className="input"
      ...
      {error ? (
        ...
        <Panel>
          <EmptyState
            ...
            hint="Players are recorded automatically when they join the Minecraft server or when events stream in."
          />
        </Panel>
      ) : (
              ...
              <>
                    ...
                    <div>
                      ...
                      <div className="faint" style={{ fontSize: 12 }}>
                        {selected.player.online
                          ? "Online now"
                          : `Last seen ${formatRelative(selected.player.lastSeen)}`}
                      </div>
                    </div>
                  </div>
                ...
                {selected.statistics.length === 0 ? (
                  <div className="faint" style={{ fontSize: 12.5 }}>
                    No statistics reported yet — the Minecraft integration sends them during play.
                  </div>
                ) : (
                ...
                {selected.graves.length === 0 ? (
                  ...
                  selected.graves.map((g) => (
                    <div
                      key={g.id}
                      style={{ fontSize: 12.5, padding: "6px 0", borderBottom: "1px solid var(--border)" }}
                    >
                      Grave: {g.dimension} · {g.x}, {g.y}, {g.z} · {formatRelative(g.death_time)} · {g.status}
                    </div>
                  ))
                )}
