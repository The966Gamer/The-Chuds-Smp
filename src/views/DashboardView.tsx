...
import { McHead, StatusBadge, EmptyState } from "@/components/ui";
...
interface StatusPayload {
  falix?: {
    status?: string;
    message?: string;
    resources?: { cpu: number; memory: number; uptime: number } | null;
    error?: string;
  };
  minecraft?: {
    online?: boolean;
...
export default function DashboardView({ user }: { user: { username: string; role: string } }) {
  ...
  const [activity, setActivity] = React.useState<
    { id: number; type: string; message: string | null; playerName: string | null; createdAt: string }[]
  >([]);
  ...
  React.useEffect(() => {
    ...
    api
      .get<{ layout: SavedLayout | null }>("/api/layout")
      .then((r) => {
        if (r.layout?.widgets) {
          setLayout({ widgets: r.layout.widgets, hidden: r.layout.hidden ?? [] });
        }
        setLayoutReady(true);
      })
        ...
        if (names.length > 0) {
          api
            ...
            .then((r) =>
              setPlayers(
                r.players
                  .filter((p) => names.some((n) => n.toLowerCase() === p.username.toLowerCase()))
                  .map((p) => ({ ...p, online: true })),
              ),
            )
            .catch(() => undefined);
  ...
  useRealtime((type, payload) => {
    if (type === "console" && payload && typeof payload === "object" && "lines" in payload) {
  ...
  const falixStatus = status?.falix && !status.falix.error ? status.falix.status ?? "unknown" : "unknown";
  const mcOnline = status?.minecraft?.online === true;
  const online = status?.players?.onlinePlayers ?? (mcOnline ? status?.minecraft?.playersOnline ?? 0 : 0);
  const max = status?.minecraft?.playersMax ?? null;
  const powerScope = me?.powerScope ?? (user.role === "admin" ? "full" : "none");
  ...
  function onMouseDown(id: string, mode: "move" | "resize") {
    return (e: React.MouseEvent) => {
      ...
      dragState.current = {
        ...
        startX: e.clientX,
        startY: e.clientY,
        orig: layout.widgets?.[id] ?? DEFAULT_LAYOUT[id as WidgetId],
        mode,
      };
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: 18 }}>
        <h1 style={{ fontSize: 20 }}>Welcome back, {me?.user.username || user.username}</h1>
      ...
      {headNotice ? (
        <div
          ...
          style={{
            padding: "10px 14px",
            marginBottom: 14,
            borderColor: "rgba(224, 168, 60, 0.4)",
        ...
        >
          ...
          <span style={{ fontSize: 13 }}>
            Head data not found. Please log into the Minecraft server once, then refresh.
          </span>
          <button className="btn sm ghost" style={{ marginLeft: "auto" }} onClick={() => setHeadNotice(false)}>
            Dismiss
          </button>
        </div>
      ) : null}
      ...
      <div className="spread" style={{ marginBottom: 12 }}>
        <div className="row" style={{ gap: 8, flexWrap: "wrap" }}>
          ...
          {editMode ? (
            <>
              <button className="btn sm" onClick={() => saveLayout({ widgets: DEFAULT_LAYOUT, hidden: [] })}>
                Restore defaults
              </button>
          ...
          return (
            ...
            >
              <div
                ...
                onMouseDown={onMouseDown(w.id, "move")}
                ...
                {editMode ? (
                  <div className="row" style={{ gap: 6 }} onMouseDown={(e) => e.stopPropagation()}>
                    <button
                      ...
                      style={{ cursor: "nwse-resize", padding: "2px 6px" }}
                      onMouseDown={onMouseDown(w.id, "resize")}
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 10, height: "100%" }}>
        ...
        <div className="row" style={{ gap: 14, fontSize: 12.5, color: "var(--text-dim)", flexWrap: "wrap" }}>
          ...
          {status?.falix?.resources?.memory ? (
            <span>🧠 {(status.falix.resources.memory / 1024 / 1024).toFixed(0)} MB</span>
          ) : null}
        </div>
        {status?.falix?.error ? (
          <div style={{ fontSize: 12, color: "var(--warn)" }}>Falix: {status.falix.error}</div>
        ) : null}
      </div>
    );
  }
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10, height: "100%", justifyContent: "center" }}>
        ...
        {!me || me.user.role !== "admin" ? (
          <div className="faint" style={{ fontSize: 11.5 }}>
            Server controls require administrator permissions.
          </div>
        ) : null}
      </div>
    );
    ...
    return (
      ...
      >
        <div style={{ fontSize: 34, fontWeight: 800, letterSpacing: "-0.02em" }}>
          ...
          <span className="dim" style={{ fontSize: 17, fontWeight: 600 }}>
            {" "}
            / {max ?? "?"}
          </span>
        </div>
        <div className="faint" style={{ fontSize: 11.5, textTransform: "uppercase", letterSpacing: "0.08em" }}>
          players online
        </div>
        {max != null && max > 0 ? (
          <div
          ...
          >
            <div
              style={{
                width: `${Math.min(100, (online / max) * 100)}%`,
                height: "100%",
    ...
    if (players.length === 0) {
      return (
        <EmptyState
          ...
          hint="Players appear here as soon as they join the server."
        />
      );
    }
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {players.map((p) => (
          <div
            key={p.username}
            className="row"
            style={{ padding: "6px 8px", borderRadius: 4, background: "rgba(148,163,184,0.05)" }}
          >
            <McHead username={p.username} headUrl={p.headUrl} size={30} />
            <div className="grow">
              <div style={{ fontSize: 13.5, fontWeight: 600 }}>{p.username}</div>
              <div className="faint" style={{ fontSize: 11.5 }}>
                Playtime {formatDuration(p.playtimeSeconds)}
              </div>
            </div>
    ...
    if (activity.length === 0) {
      return (
        <EmptyState
          ...
          hint="Joins, deaths and server events will show up here."
        />
      );
    }
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
        {activity.map((e) => (
          <div key={e.id} className="row" style={{ fontSize: 13 }}>
            ...
            <span className="faint" style={{ fontSize: 11.5, flexShrink: 0 }}>
      ...
      {consoleLines.length === 0 ? (
        ...
        consoleLines.map((l, i) => (
          <div
            key={i}
            style={{
              color: /error|exception/i.test(l) ? "#f08a85" : /warn/i.test(l) ? "var(--warn)" : "var(--text-dim)",
            }}
          >
            {l}
          </div>
...
function EventIcon({ type }: { type: string }) {
  ...
  return (
    <PixelIconView
      name={e.icon}
      size={15}
      style={{ opacity: 0.9, filter: `drop-shadow(0 0 3px ${e.color})` }}
    />
  );
}
