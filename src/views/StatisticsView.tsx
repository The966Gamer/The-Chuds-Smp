...
const CATEGORIES: { label: string; icon: string; match: RegExp }[] = [
  ...
  { label: "Blocks placed", icon: "🧱", match: /^minecraft:used/ },
  {
    ...
    icon: "🧭",
    match:
      /^minecraft:walk_one_cm|^minecraft:sprint|^minecraft:swim|^minecraft:fly|^minecraft:fall|^minecraft:crouch|^minecraft:boat|^minecraft:minecart|^minecraft:horse|^minecraft:pig|^minecraft:strider|^minecraft:aviate/,
  },
        ...
        <>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    ...
                    {rows.length > top.length ? (
                      <div className="faint" style={{ fontSize: 11.5 }}>
                        + {rows.length - top.length} more in this category
                      </div>
                    ) : null}
                  </div>
                </Panel>
          ...
          {data.statistics.length === 0 ? (
            <Panel>
              <EmptyState
                icon="statistics"
                title="No statistics reported yet"
                hint="They will appear here after the Minecraft integration sends your first statistics payload."
              />
            </Panel>
          ) : null}
        </>
